...
export default function HomeClient({ initialData }: { initialData: any[] | null }) {
  ...
  // We can use the server-fetched data here if needed
            ...
            >
              <div className="relative z-10 p-3 sm:p-6 lg:p-10 h-full flex flex-col justify-between">
                ...
                <motion.a
                  onClick={handleClick2}
                  whileHover={{ x: 5 }}
                  className="inline-flex items-center gap-1 sm:gap-3 text-white font-bold text-xs sm:text-xl cursor-pointer"
                >
          ...
          >
            {[
              ...
              >
                ...
                <h4 className="text-2xl font-bold mb-3" style={{ color: "var(--text)" }}>{feature.title}</h4>
                <p className="opacity-70" style={{ color: "var(--text)" }}>{feature.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="relative z-10 text-center">
              <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} viewport={{ once: true }} className="inline-block mb-6">
                <Award className="w-15 h-15 text-white mx-auto" />
              </motion.div>
              <h2 className="text-4xl md:text-4xl font-black mb-6 text-white">{t.ctaTitle || "Ready to Build Something Amazing?"}</h2>
              <p className="text-xl text-white/80 mb-8 max-w-1xl mx-auto">{t.ctaDesc || "Join hundreds of satisfied clients"}</p>
              <motion.button
                onClick={handleClick}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
