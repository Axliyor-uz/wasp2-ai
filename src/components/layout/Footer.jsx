...
export default function Footer() {
  ...
  return (
    <footer className={`relative transition-colors duration-500 ${isDark ? "bg-gray-900" : "bg-gray-50"}`}>
      ...
      <div className="max-w-[1440px] mx-auto px-6 py-20 lg:py-28"> 
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-16">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-xl ${isDark ? "bg-purple-500/20" : "bg-purple-100"}`}>
                <Briefcase className={`w-8 h-8 ${isDark ? "text-purple-400" : "text-purple-600"}`} />
              </div>
              <span className={`text-3xl font-black tracking-tight ${isDark ? "text-white" : "text-gray-900"}`}>
          ...
          {/* Quick Links */}
          <div>
            <h3 className={`text-xl font-bold mb-8 uppercase tracking-widest ${isDark ? "text-white" : "text-gray-900"}`}>
              {t.quickLinks}
            </h3>
            <ul className="space-y-4">
              {quickLinks.map((link, index) => (
                <li key={index}>
                  <Link
                    href={link.href}
                    className={`flex items-center gap-3 text-base lg:text-lg font-medium hover:text-purple-500 transition-all hover:translate-x-1 ${isDark ? "text-gray-400 hover:text-purple-400" : "text-gray-600 hover:text-purple-600"}`}
                  >
          ...
          <div>
            ...
            <ul className="space-y-4">
              {services.map((service, index) => (
                <li key={index}>
                  <span className={`text-base lg:text-lg font-medium block ${isDark ? "text-gray-400" : "text-gray-600"}`}>
                    {service.label}
                  </span>
                </li>
              ))}
          ...
          <div>
            ...
            <p className={`text-base lg:text-lg mb-8 leading-relaxed ${isDark ? "text-gray-400" : "text-gray-600"}`}>
              {t.newsletterDesc}
            </p>
            ...
            <div className="space-y-5">
              <div className="flex items-center gap-4">
                ...
                <span className={`text-base lg:text-lg font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>{t.phone}</span>
              </div>
              <div className="flex items-center gap-4">
                <Mail className={`w-5 h-5 ${isDark ? "text-purple-400" : "text-purple-600"}`} />
                <span className={`text-base lg:text-lg font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>{t.email}</span>
              </div>
              <div className="flex items-center gap-4">
                <MapPin className={`w-5 h-5 ${isDark ? "text-purple-400" : "text-purple-600"}`} />
                <span className={`text-base lg:text-lg font-medium ${isDark ? "text-gray-300" : "text-gray-700"}`}>{t.address}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-20 pt-10 border-t border-gray-700/30">
          <div className="flex flex-col md:flex-row justify-between items-center gap-10">
            {/* Social Links */}
            <div>
              <h4 className={`text-sm font-bold mb-5 uppercase tracking-tighter text-center md:text-left ${isDark ? "text-gray-500" : "text-gray-400"}`}>
                {t.social}
              </h4>
              <div className="flex gap-4">
                {socialLinks.map((social, index) => (
                  <a
                    ...
                    className={`p-3 rounded-xl border-2 transition-all hover:scale-110 ${isDark ? "bg-gray-800 border-gray-700 text-gray-300 hover:text-white hover:border-purple-500" : "bg-white border-gray-200 text-gray-600 hover:text-purple-600 hover:border-purple-400"}`}
            ...
            {/* Copyright */}
            <div className={`text-base lg:text-lg text-center md:text-right font-medium ${isDark ? "text-gray-400" : "text-gray-600"}`}>
              <p>{t.copyright}</p>
